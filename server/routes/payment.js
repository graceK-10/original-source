...
async function markOrderPaidAndNotify(order, paymentDetails = {}) {
  ...
  try {
    await sendOrderWhatsappReceipt(updatedOrder);
    console.log("✅ Sent WhatsApp receipt for", updatedOrder.orderId);
  } catch (e) {
  ...
  try {
   ...
   if (!m_tx_id || !m_tx_amount) {
    ...
    const formData = {
      ...
      m_return_url: m_return_url || process.env.INSTAPAY_SUCCESS_URL || "http://localhost:4040/payment-success",
      m_notify_url: m_notify_url || process.env.INSTAPAY_NOTIFICATION_URL || "http://localhost:4040/api/payment/notifications",
      m_back2shop_url: m_back2shop_url || process.env.INSTAPAY_SHOP_URL || "http://localhost:4040/products",
...
router.post("/notifications", async (req, res) => {
  try {
    ...
    const status = statusMap[body.requestStatus] || "failed";
...
const reference = body.payeeRefInfo;
    ...
    if (status !== "completed" && existingOrder) {
      await updateOrderById(existingOrder.orderId, {
        $set: {
          "payment.status": status,
          status,
          "payment.transactionRef": body.paymentSystemReference || existingOrder?.payment?.transactionRef || null,
          "payment.method": existingOrder?.payment?.method || body.paymentMethod || "INSTAPAY",
        },
      });
    }
...
router.post("/confirm-success", async (req, res) => {
  try {
    const { orderId, paymentStatus, paymentId, transactionId, amount, rawParams } = req.body || {};
    ...
    const looksCompleted = ["COMPLETE", "COMPLETED", "SUCCESS", "PAID"].includes(normalizedStatus);

    if (!looksCompleted) {
      const pendingOrder =
        (await updateOrderById(order.orderId, {
          $set: {
            "payment.status": String(order?.payment?.status || "pending").toLowerCase(),
